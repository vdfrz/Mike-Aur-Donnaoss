...
const LEGAL_DRAFTING_PROMPT: &str = r#"
LEGAL DRAFTING MODE — ACTIVE

You are a senior advocate at this firm. You draft documents as they have been filed in the firm's actual cases: Rohini (North) Courts, Dwarka (South-West) Courts, PHC (Punjab & Haryana High Court), AFT, Delhi High Court, Tis Hazari, Labour Courts.

ABSOLUTE OUTPUT RULES:
- Begin the document immediately — no preamble, no explanation.
- ZERO underscores, ZERO [BRACKETS], ZERO blank fields.
- DATA FALLBACK: If a name, Aadhar card number, Voter ID, or address is not provided by the user, you MUST use "000000" or "0000 0000 0000" instead. Never leave it blank.
- Style: "S/o Sh.", "W/o Mr.", "R/o", "u/S", numbered paragraphs starting with "That, ".
- Use RAG context to mirror cases this firm has already filed.
...
══════════════════════════════════════════════
STATUTE & COURT MATCHING
══════════════════════════════════════════════
• Maintenance → Section 125 CrPC, HMA 1955, Rajnesh v. Neha (2021) 2 SCC 324
• Domestic Violence → PWDVA 2005 Sections 12, 18-23
• Divorce → Section 13 B HMA 1955
• Military → Armed Forces Tribunal Act 2007
CLOSE ALL DOCUMENTS WITH:
PRAYER:
It is therefore most respectfully prayed that this Hon'ble Court may graciously be pleased to:
(i) [Primary relief]
(ii) Pass such other order(s) as this Hon'ble Court may deem fit and proper.
And for this act of kindness, the Petitioner/Complainant shall ever pray.
                                        PETITIONER/COMPLAINANT/DEPONENT
                                        Through Counsel
"#;
...
async fn stream_chat_root(
    ...
    let ik_prompt = build_ik_system_prompt(&ik_results);
    let mut sections: Vec<String> = vec![MIKE_SYSTEM_PROMPT.trim().to_string()];
    sections.push(LEGAL_DRAFTING_PROMPT.trim().to_string());
                ...
                Ok(mut s) => {
                    ...
                    if iter_tool_calls.is_empty() {
                        ...
                        let echoed_tool = if lower.contains("generate_docx") && lower.contains("\"body\"") {
                            // Try to parse the outermost JSON object the model emitted.
                            let trimmed = full_response.trim();
                            let start = trimmed.find('{').unwrap_or(0);
                            serde_json::from_str::<serde_json::Value>(&trimmed[start..]).ok()
                                .and_then(|v| {
                                    // Support both {"name":…,"parameters":{…}} and {"name":…,"input":{…}}
                        ...
                        if let Some((title, body)) = echoed_tool {
                            // Suppress raw JSON from the chat stream
                        ...
                        } else if keywords.iter().any(|k| lower.contains(k)) && !full_response.contains("doc_id") {
