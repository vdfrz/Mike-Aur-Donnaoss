# Agent Prompt Tightening + Demo Blur Toggle — Batch Prompt

**Scope:** Two changes:
1. `src/agents/case_prep/mod.rs` — tighten all 7 agent prompts based on real Indian legal document analysis
2. `frontend/src/app/cases/[id]/page.tsx` — add a demo-mode party name blur toggle

**Do NOT copy code from Lavern or any external project. Do NOT change the Rust backend beyond `mod.rs`. Do NOT install new packages.**

---

## Part A: Factor Out the Shared Context Block

Currently, the identical ~40-line "INDIAN LEGAL CONTEXT" block is duplicated verbatim across all 7 agent constants (`CASE_SUMMARY_AGENT`, `STRENGTHS_WEAKNESSES_AGENT`, `EVIDENCE_GAP_AGENT`, `OPPOSITION_PREDICTOR`, `STRATEGY_RECOMMENDER`, `PRECEDENT_FINDER`, `RISK_ASSESSOR`).

**Required refactor:** Extract the shared block into a single constant and concatenate at compile time.

```rust
// At the top of mod.rs:
const INDIAN_LEGAL_CONTEXT: &str = r#"INDIAN LEGAL CONTEXT — apply throughout.
... (the tightened block from Part B below)
---"#;

// Then each agent uses:
// We can't concat! raw strings at compile time in stable Rust,
// so use lazy_static or just keep the shared block as a separate const
// and concatenate in the orchestrator before passing to StreamParams.
```

**If compile-time concatenation is awkward**, the alternative is: keep the shared block as `pub const INDIAN_LEGAL_CONTEXT: &str` and modify `orchestrator.rs` line 174 from:
```rust
let mut full_prompt = system_prompt.to_string();
```
to:
```rust
let mut full_prompt = format!("{}\n\n{}", INDIAN_LEGAL_CONTEXT, system_prompt);
```
Then remove the duplicated context block from each agent constant, leaving only the agent-specific instructions.

**Choose whichever approach is simpler.** The goal is: one place to update the shared context, not seven.

---

## Part B: Tightened Indian Legal Context Block

Replace the current shared context block with this. Changes are marked with `// NEW` or `// CHANGED` comments (remove these comments in the actual code).

```
INDIAN LEGAL CONTEXT — apply throughout.
...
## Part C: Tightened Per-Agent Instructions

Each agent constant should contain ONLY the agent-specific instructions below (the shared context is now in `INDIAN_LEGAL_CONTEXT`). Keep the JSON schema and grounding rules exactly as they are currently, but add the new instructions below to each agent.

### C1. CASE_SUMMARY_AGENT — Add These Instructions
...
### C2. STRENGTHS_WEAKNESSES_AGENT — Add These Instructions
...
### C3. EVIDENCE_GAP_AGENT — Add These Instructions
...
### C4. OPPOSITION_PREDICTOR — Add These Instructions
...
### C5. STRATEGY_RECOMMENDER — Add These Instructions
...
### C6. PRECEDENT_FINDER — Add These Instructions
...
### C7. RISK_ASSESSOR — Add These Instructions

**Also expand the `risk_type` enum** to include the additional types already partially present:
...
## Part D: Demo Mode — Party Name Blur Toggle

### D1. Frontend State + Toggle UI

In `frontend/src/app/cases/[id]/page.tsx`:
...
**Add toggle button** in the case header area (near the case title, top-right). Use existing design tokens:
...
>
  <Eye className="h-3 w-3" /> {/* or EyeOff when active */}
  {demoMode ? "Demo Mode" : "Demo"}
</button>
```

Import `Eye` and `EyeOff` from `lucide-react` (already installed). Show `EyeOff` when `demoMode` is true, `Eye` when false.

### D2. Party Name Collection

The case_summary finding already produces a `parties` object: `{ petitioner, respondent, other[] }`. Also, the case has `parties_json` on the case info.

Create a `useMemo` that collects all party name strings:
...
const blurNames = useMemo(() => {
  ...
  // From case parties_json
  for (const p of parties) {
    if (p.name) names.push(p.name);
  }

  // From case_summary finding
  if (summaryFinding) {
    try {
      const content = JSON.parse(summaryFinding.content_json);
  ...
  // Deduplicate and filter short names (avoid blurring "v." or "The")
  return [...new Set(names)].filter(n => n.length > 3);
}, [demoMode, parties, summaryFinding]);
```

### D3. Blur Application — CSS Approach

**Do NOT use regex replacement on rendered text.** Instead, create a utility function and a wrapper component:

```tsx
function blurPartyNames(text: string, names: string[]): React.ReactNode {
  if (names.length === 0) return text;

  // Build a regex matching any party name (case-insensitive, longest first)
...
**Critical:** The blurred text must:
- Use the SAME font (font-serif for content, font-sans for UI labels)
- Use the SAME font size
- Take up the SAME space (no layout shift) — `blur()` CSS filter achieves this naturally
- Be `select-none` so copying doesn't grab the name
- NOT change the surrounding layout at all

### D4. Where to Apply Blurring

Pass `blurNames` array and the `blurPartyNames` utility to **every** location where party names can appear. This includes places people forget — the docket line is the most important one for demos.

1. **Sidebar — case title** (line ~895, `{caseInfo.title}`): Case titles are often "Sharma v. Singh" — blur the party name portions. Apply `blurPartyNames(caseInfo.title, blurNames)` inside the `<h2>`.
2. **Sidebar — parties list** (lines ~944-953): Blur `{p.name}` for each party. Keep the role label ("Petitioner:", "Respondent:") visible — only the name blurs.
3. **Sidebar — court / docket line** (line ~929, `{caseInfo.court}`): If the court string contains party names (rare but possible in tribunal cause titles), blur them.
4. **Overview tab — case summary "Parties" card** (lines ~99-102): Blur in "petitioner v. respondent" text.
5. **Overview tab — case summary `factual_background`**: Party names appear throughout the narrative. Blur them.
6. **Finding cards — `renderFindingContent`**: Apply to ALL text content in findings — strengths/weaknesses points, evidence gap descriptions, opposition predictions, strategy recommendations, risk descriptions. Party names appear in quotes, grounding text, and narrative.
7. **Chat tab — assistant message content**: Apply to assistant message text.
8. **Output generation results**: If outputs display generated text containing party names, blur there too.

**Do NOT blur:**
- Tab labels, section headers, agent names
- Button text, UI chrome, the "Demo" toggle itself
- Document filenames in the sidebar
- Role labels ("Petitioner:", "Respondent:", "OP-1:") — only the name after the role
- The case number itself (e.g., "HMA/1101/22") — that's not a party name

### D5. Passing Context

Thread `blurNames` and `blurPartyNames` through as props or via the existing component structure. Since everything is in one file (`page.tsx`), this is just passing the array to the relevant render sections.

For the `renderFindingContent` function: wrap its text outputs with the blur utility. For ReactMarkdown content (in ChatTab), use a custom renderer that applies blurring to text nodes:
...
1. `src/agents/case_prep/mod.rs` — THE file being modified (all 7 agent prompts)
2. `src/agents/case_prep/orchestrator.rs` — how prompts are assembled and passed to the LLM (especially line 168-182)
3. `frontend/src/app/cases/[id]/page.tsx` — THE frontend file being modified (demo blur toggle)
4. `frontend/src/app/components/shared/types.ts` — CaseFinding, AnalysisProgress types

## Constraints

- **Do NOT create new files** unless factoring out `INDIAN_LEGAL_CONTEXT` requires a new Rust module (it shouldn't — keep it in `mod.rs`).
- **Do NOT install new packages.** Everything needed (`lucide-react`, `react-markdown`, etc.) is already installed.
- **Do NOT change the Rust backend** beyond `src/agents/case_prep/mod.rs` (and the one-line change to `orchestrator.rs` if factoring out the shared block).
- **Do NOT copy code from Lavern** or any external project.
- **Match existing style exactly.** Tailwind tokens, fonts, colors per CASES_BEAUTIFICATION_PROMPT.md.
- **Keep all JSON output schemas identical.** The tightening adds INSTRUCTIONS, not schema changes (except the `risk_type` enum expansion in RISK_ASSESSOR).
- **Keep all grounding rules.** Do not weaken them.
- **CLAUDE.md rules apply:** Surgical changes only. Every changed line traces to this spec.
- **The blur toggle must not break when no findings exist yet** (empty `blurNames` array = no blurring = normal display).
