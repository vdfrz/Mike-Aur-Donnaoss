...
export function AppSidebar({ isOpen, onToggle }: AppSidebarProps) {
    ...
    return (
        ...
        >
            {/* Toggle + Logo */}
            <div
                className={`mb-3 items-center justify-between px-2.5 py-2 ${!isOpen ? "hidden md:flex" : "flex"
                    }`}
            >
                {isOpen && (
                    <div className="px-2.5">
                        ...
                        >
                            <MikeIcon size={22} />
                            <span
                                className={`text-2xl font-light font-serif ${shouldAnimate ? "sidebar-fade-in" : ""
                                    }`}
                            >
                                Mike aur Donna
                            </span>
                ...
                return (
                    <div key={href} className="py-1 px-2.5">
                        ...
                        >
                            ...
                            {isOpen && (
                                <span
                                    className={`text-sm font-medium ${shouldAnimate ? "sidebar-fade-in-2" : ""
                                        }`}
                                >
                                    {label}
                                </span>
            ...
            {isOpen && pathname.startsWith("/assistant") && (
                <div className="mt-4 flex-1 min-h-0 flex flex-col">
                    <button
                        ...
                        className={`mb-2 px-5 flex items-center justify-between text-xs font-semibold text-gray-500 hover:text-gray-700 transition-colors ${shouldAnimate ? "sidebar-fade-in" : ""
                            }`}
                    >
                    ...
                    >
                        {!chats ? (
                            ...
                            <div
                                className={`text-xs text-gray-500 py-2 px-5 ${shouldAnimate ? "sidebar-fade-in-2" : ""
                                    }`}
                            >
                                {tSidebar("noChats")}
                            </div>
                            ...
                            <div
                                className={`space-y-1 px-2.5 ${shouldAnimate ? "sidebar-fade-in-2" : ""
                                    }`}
                            >
                                {chats.map((chat) => (
                                    <SidebarChatItem
                ...
                {user && (
                    <div className="relative">
                        <button
                            ...
                            className={`flex items-center transition-colors w-full px-3.5 py-4 border-t border-gray-200 ${!isOpen ? "hidden md:flex" : ""
                        ...
                        >
                            ...
                            {isOpen && (
                                <div
                                    className={`text-left flex-1 min-w-0 pl-3 flex items-center justify-between gap-2 ${shouldAnimate ? "sidebar-fade-in-2" : ""
                                        }`}
                                >
